...
object state {
  ...
  object ZSettlementState {

    def updateSettlements(settlements: Map[Vertex, Settlement]) = ZIO.updateState[SOCSettlementMap](_ => SOCSettlementMap(settlements))

    def settlementsForPlayer(player: Int) = ZIO.getState[SOCSettlementMap].map(_.values.count(_.playerId == player))
